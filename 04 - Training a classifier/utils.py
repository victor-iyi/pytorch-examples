...
def imshow(images, labels, pred=None, **kwargs):
    ...
    === Keyword Arguments ===
        normalize: float, default 0.5
            Batch normalization value. to un-normalize the data, if the data
            has already been normalize, if not just set normalize to a falsy value.
            e.g normalize=False or normalize=None.
    ...
    elif type(classes) == list or type(classes) == range:
        pass
    elif type(classes) == tuple or type(classes) == set:
        pass
    else:
...
def accuracy(net:nn.Module, data:torch.utils.data.DataLoader, **kwargs):
    ...
    if not ret or logging:
        ...
        # Overall accuracy
        print(35 * '–')
        print(f'Overall accuracy \t = {accuracy.mean():.2%}')
        print(35 * '–')    
